...
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                             ...
                                             prefetch_factor=2, persistent_workers=False) # 8 workers may work faster
              for x in ['train', 'val']}
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # Iterate over data.
            for iter, data in tqdm(enumerate(dataloaders[phase]), total=len(dataloaders[phase])):
                # get the inputs
                inputs, labels = data
            ...
            if phase == 'val':
                ...
                save_network(model, int(epoch_acc*1000) / 10)
                draw_curve(epoch)
            if phase == 'train':
               scheduler.step()
...
optim_name = optim.Adam #apex.optimizers.FusedSGD
